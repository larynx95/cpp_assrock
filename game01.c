/**********************************************************
How to Create a Simple Ninja Video Game in C
https://www.youtube.com/watch?v=t9WCBVNoKaQ
https://gist.github.com/fernandozamoraj/4a996896be37ac89b3e0cf346931c75b
**********************************************************/
...
void set_game_state_over() { game_state = GAME_STATE_OVER; }
...
void increment_score() { score += GOAL_POINTS; }
...
void clean_up() { printf("Thanks for playing."); }
...
void update_ninja(char ch) {
  ...
  if (ninja_x == 1 && ch == 'j' && game_state == GAME_STATE_PLAYING) {
    ...
    ninja_x += ninja_delta;
    increment_score();
  } else if (ninja_x == SCREEN_WIDTH - 1 && ch == 'j' &&
             game_state == GAME_STATE_PLAYING) {
    ninja_delta = -NINJA_SPEED;
    ninja_x += ninja_delta;
    increment_score();
